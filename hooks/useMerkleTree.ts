...
import { keccak256, encodeAbiParameters, Address, Hex, isHex, getAddress } from 'viem';
...
const encodeLeaf = (address: string, amount: string): string =>
  keccak256(encodeAbiParameters([{ type: 'address' }, { type: 'uint128' }], [getAddress(address), BigInt(amount)]));
