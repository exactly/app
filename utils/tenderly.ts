...
export const transferDAI = async (address: string, amount: number) => {
  ...
  const tokenAmount = ethers.utils.hexValue(ethers.utils.parseEther(amount.toString()).toHexString());

  const unsignedTx = await daiContract.populateTransaction.approve(await signer.getAddress(), tokenAmount);
  ...
  await daiContract.transferFrom(AddressZero, address, tokenAmount);
};
